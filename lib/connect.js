...
const connectInternal = (connect, resolve) => {
    const { type = 'base', host, port, data, info, timeout = 30000, connectTimeout = timeout, dataTimeout = timeout, certificate = false, download = true, ssl: _ssl = false, tls: _tls = _ssl, client = _tls ? tls : net, ...options } = connect
    ...
    const socket = client.connect({ ...options, port, host })
